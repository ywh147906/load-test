...
//无gc分配原理（以slice为例）：
/*
//调用NewSlice时，转换为slice裸类型
type slice struct {
    data unsafe.Pointer
    cap int
    len int
}
//根据len计算slice的data部分需要的byte容量
//创建对应容量chuck对象，实际为bytes数组
//将slice的data指向chuck对象，返回

//由于分配内存用的是一个bytes数组，可以绕开golang垃圾回收机制，规避掉slice中指针的gc扫描
*/
