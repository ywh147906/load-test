...
func (r *runner) spawnWorkersWithInitFn(spawnCount int, quit chan bool, spawnCompleteFunc func()) {
	log.Println("Spawning-with-init-fn", spawnCount, "clients immediately")
...
func (r *runner) startSpawning(spawnCount int, spawnRate float64, spawnCompleteFunc func()) {
	...
	//r.numClients = 0
	if SpawnWithInitFunc {
		go r.spawnWorkersWithInitFn(spawnCount, r.stopChan, spawnCompleteFunc)
	} else {
...
func (r *slaveRunner) sumUsersAmount(msg *genericMessage) int {
	...
	if r.userClassesCountFromMaster == nil {
	...
	for class, num := range userClassesCountMap {
		...
		amount = int(n - r.userClassesCountFromMaster[c])
